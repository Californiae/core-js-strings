 ...
 */
function removeLastOccurrences(s, v) {
  const index = s.lastIndexOf(v);
  if (index === -1) {
    return s;
  }
  return s.slice(0, index) + s.slice(index + v.length);
}
...
function reverseString(s) {
  return s === undefined || s === '' ? '' : s.split('').reverse().join('');
}
 ...
 */
function orderAlphabetically(s) {
 ...
 */
function containsSubstring(s, b) {
  return s.includes(b);
}
...
function isPalindrome(str) {
  const tr = str.replace(/[^a-z0-9]/gi, '').toLowerCase();
  const r = tr.split('').reverse().join('');
  return tr === r;
}
...
function reverseWords(s) {
  const n = s.split(' ');
  return n.map((i) => i.split('').reverse().join('')).join(' ');
}
...
function encodeToRot13(s) {
  ...
  const m = s
    .split('')
    .map((i) => {
      return a.indexOf(i) > -1 ? b[a.indexOf(i)] : i;
    })
    .join('');
  return m;
}
